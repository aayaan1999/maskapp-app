(() => {
  ...
  async function extractFields(file) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 second upload timeout
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setStatus(uploadStatus, data.error || "Failed to upload file.", "error");
      ...
      // File uploaded! Now start polling for processing
      setStatus(uploadStatus, "Processing document (this may take a few minutes)…", "loading");
      pollForProcessing();
      
    } catch (err) {
      clearTimeout(timeoutId);
      if (err.name === 'AbortError') {
        setStatus(uploadStatus, "Upload took too long. Please check your connection and try again.", "error");
      } else {
  ...
  async function pollForProcessing() {
    /**
     * Poll the /extract/status endpoint until processing is complete.
     * This handles the async background processing gracefully.
     */
    const maxPolls = 1200; // 20 minutes with 1-second polling
    let pollCount = 0;
    ...
    while (pollCount < maxPolls) {
      try {
        ...
        if (data.status === "complete") {
          // Success! Load the UI with results
          renderPreviews(data);
          renderGroups(data);
          ...
          setStatus(uploadStatus, data.error || "Processing failed. Please try again.", "error");
          dropzone.classList.remove("dropzone--busy");
          return;
        }
        
        // Still processing - show progress
        const processingTime = Math.floor(data.processing_time || 0);
        const minutes = Math.floor(processingTime / 60);
        const seconds = processingTime % 60;
        
        let timeStr = "Processing";
        if (minutes > 0) {
          timeStr = `Processing (${minutes}m ${seconds}s elapsed)`;
        } else if (processingTime > 0) {
          timeStr = `Processing (${seconds}s elapsed)`;
        }
        
        setStatus(uploadStatus, timeStr + "…", "loading");
        ...
        if (timeSinceProgress > 5000) {
          ...
          setStatus(uploadStatus, "Checking status (connection issue)…", "loading");
        }
    ...
    // Max polling time exceeded
    setStatus(uploadStatus, "Processing took too long. The server may be busy. Please try again later.", "error");
    dropzone.classList.remove("dropzone--busy");
  }
  ...
  maskBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        setStatus(maskStatus, "Masking took too long. Please try again.", "error");
      } else {
