(() => {
  ...
  async function extractFields(file) {
    ...
    // FIXED: Added AbortController with 10-minute timeout
    ...
    try {
      const res = await fetch("/extract", { 
        method: "POST", 
        body: formData,
        signal: controller.signal  // FIXED: Added signal for timeout control
      });
      ...
      if (err.name === 'AbortError') {
        setStatus(uploadStatus, "Processing took too long. Try uploading a smaller PDF or check your connection.", "error");
      } else {
  ...
  maskBtn.addEventListener("click", async () => {
    ...
    try {
      // FIXED: Removed undefined variables and simplified payload construction
      const payload = {
        job_id: currentJobId,
        group_ids: selected,
        instructions
      ...
      // FIXED: Added AbortController with timeout for mask request too
      const controller = new AbortController();
      ...
      // FIXED: Better error message for timeout
      if (err.name === 'AbortError') {
